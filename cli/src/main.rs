...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Client { action }) => commands::client::handle(action),
        Some(Commands::Project { action }) => commands::project::handle(action),
        Some(Commands::Template { action }) => commands::template::handle(action),
